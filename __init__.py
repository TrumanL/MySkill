            ...
            for i in range(len(fullData["messages"])):
              ...
              if poppedData["response-needed"] == "True":
                ...
                if any(word in outMessageConfirm for word in yes_words):
                    ...
                    self.emitTest.wait_for_response(Message.deserialize(json.dumps({"type":"skill-audio-record:AudioRecordSkillIntent", "data":{"utterance":"record audio 40 seconds"}})), 'mycroft.skill.handler.complete')
                    self.speak('Done')
                    wait_while_speaking()
