...
class MySkill(MycroftSkill):
  ...
  def __init__(self):
    ...
    self.testMessage = Message.deserialize(json.dumps({"type":"NULL", "data":{"messageData":{"messages" ["hi", "sender":"bob","response-needed":"True"]}s}}))
  def initialize(self):
            ...
            for i in range(len(fullData["messages"])):
              ...
              if poppedData["response-needed"] == "True":
                ...
                if any(word in outMessageConfirm for word in yes_words):
                    ...
                    self.emitTest.emit('mycrorft-audio-record')
                    self.speak('Done')
                    wait_while_speaking()
